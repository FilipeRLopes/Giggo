...
export function Header() {
    return (
        <View className="sticky top-0 z-40 border-b border-border bg-background/80 px-4 sm:px-6 h-16 flex-row items-center justify-between">
            {/* Esquerda: Botão Menu (Mobile) / Logo (Mobile) / Título (Desktop) */}
            <View className="flex-row items-center gap-3">
                <Pressable className="h-10 w-10 lg:hidden flex items-center justify-center rounded-md hover:bg-accent hover:text-accent-foreground">
            ...
            {/* Direita: Botão Botificações */}
            <Pressable className="h-10 w-10 flex items-center justify-center rounded-md hover:bg-accent hover:text-accent-foreground">
